...
class SetMap(QWidget):

    def __init__(self):
        """地图属性初始化"""
        super(SetMap, self).__init__()
        # 地图初始大小
        self.size_x = 5
        self.size_y = 5
        # 地图储存
    ...
    def init_set(self):
        """创建初始地图"""
        positions = [(i, j) for i in range(self.size_x)
    ...
    def set(self, positions, name):
        ...
        for position, name in zip(positions, name):
            ...
            # 设置按钮与网格大小相同
            button.setMinimumSize(QtCore.QSize(
                self.grid.width, self.grid.highth))
